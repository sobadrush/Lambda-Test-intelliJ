 ...
 * @date 2022/5/14
 */
public class StreamGroupingBy_004 {
    ...
    public static void main(String[] args) {
        ConcurrentMap<String, Integer> empGroup = EMP_LIST.stream().collect(
                Collectors.groupingByConcurrent(
                        Employee::getCity,
                        Collectors.collectingAndThen(
                                Collectors.summingDouble(Employee::getSalary), numDouble -> numDouble.intValue()))
        );
        System.out.println("empGroup = " + empGroup);
        System.out.println("＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝");
        var summingTaipei = empGroup.get("Taipei");
        System.out.println("summingTaipei = " + summingTaipei);
    }
